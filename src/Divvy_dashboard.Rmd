...
library(tidyverse)
library(plotly)
p_load(flexdashboard, leaflet, leaflet.extras, dplyr, ggvis, here, conflicted, data.table, jsonlite, kableExtra, glue, geosphere, DT, networkD3, htmltools, geojsonio, data.table, magrittr, circlize)
...
station_wards <- setNames(station_wards$`FROM STATION ID`, as.character(station_wards$Wards))



# trips_mat <- divvy_trips %>% 
#   select(`FROM STATION ID`, `TO STATION ID`) %>% 
#   arrange(as.numeric(`FROM STATION ID`), as.numeric(`TO STATION ID`)) %>% 
#   table()
...
chorddiag(trips_mat[0:20,0:20], type = "bipartite", showTicks = F, groupnameFontsize = 14, groupnamePadding = 10, margin = 90)
